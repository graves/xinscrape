defmodule Xinscrape.Base do
  ...
  def process_url(endpoint) do
    "https://6um.digitalgangster.com/" <> endpoint
  end
  ...
  def verify_response({:error, error}) do
    raise(Xinscrape.Error, message: inspect error)
  end
  ...
  def do_parse_error(message, code) do
    case code do
      @error_code_not_found ->
        raise Xinscrape.NotFoundError, code: code, message: message
      _ ->
        raise Xinscrape.Error, code: code, message: message
    end
  end

  @doc """
  Sends unauthenticated GET request to a DigitalGangster endpoint.

  ## Examples
      Xinscrape.request({:get, "secret/route"})
  """
  ...
  @doc """
  Sends authenticated GET request to a DigitalGangster endpoint.

  ## Examples
      Xinscrape.request({:get, "secret/route", "secrettoken"})
  """
  ...
  @doc """
  Sends unauthenticated POST request to a DigitalGangster endpoint. The request
  body should be a Map.

  ## Examples
      Xinscrape.request({:post, "secret/route", %{csrf: "token"}})
  """
  ...
  @doc """
  Sends authenticated POST request to a DigitalGangster endpoint. The request
  body should be a Map.

  ## Examples
      Xinscrape.request({:post, "secret/route", %{my: "bod"}, "mytoken"})
  """
